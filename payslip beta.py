...
# SMTP_PORT = 587
# SENDER_EMAIL = "reecekhalid96@gmail.com"
# PASSWORD = ("bnamsdoikuzsizgt")  # Use environment variable for security
...
# # Function to send payslip email
# def send_payslip(employee):
#     employee_id = str(employee["Employee_ID"]).strip()  # Ensure correct format
#     employee_email = str(employee["Email"]).strip()  # Ensure proper email format
#     payslip_file = f"payslips/payslip_{employee_id}.pdf"  # Correct file reference

#     # Debugging: Confirm file exists before sending
#     if not os.path.exists(payslip_file):
#         print(f"ERROR: Payslip file '{payslip_file}' not found for {employee['Names']}. Skipping...")
#         return

#     if not employee_email or pd.isna(employee_email):
#         print(f"Skipping: No email found for {employee['Names']}")
#         return
...
#         print(f"Payslip sent to {employee_email}")
#     except Exception as e:
#         print(f"Failed to send payslip to {employee_email}. Error: {str(e)}")

# # Send payslip emails for all employees
# for _, employee in df.iterrows():
#     send_payslip(employee)
